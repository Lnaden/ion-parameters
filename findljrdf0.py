...
def LJ(r, epsi, sig):
    return 4*epsi*((sig/r)**12 - (sig/r)**6)
def exp0(r, epsi, sig):
    return numpy.exp(-LJ(r, epsi, sig)/kT)
def geo(p1, p2):
    return numpy.sqrt(p1*p2)
...
if __name__ == "__main__":
    ...
    parser.add_option("--darange", dest="sliceset", type='int', default=0, metavar="darange")
    ...
    pdb.set_trace()
    execute(nstates, qs, es, ss, useO=options.oxygen, sliceset=options.sliceset)
